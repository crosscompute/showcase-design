...
const featuredTools = [
  {
    ...
    name: "Tool 1",
    desc: "Description 1",
    demo: "Demo of a single tool",
  },
  {
    id: 2,
    name: "Tool 2",
    desc: "Description 2",
    demo: "Demo of another tool",
  },
  // Add more items as needed
];
...
const featuredToolkits = [
  {
    ...
    name: "Toolkit 1",
    desc: "Description 1",
    demo: "Demo of using multiple tools",
  },
  {
    id: 2,
    name: "Toolkit 2",
    desc: "Description 2",
    demo: "Demo of another toolkit",
  },
  {
    id: 3,
    name: "Toolkit 3",
    desc: "Description 3",
    demo: "Demo of yet another toolkit",
  },
  // Add more items as needed
];
  ...
  filteredTools.forEach((tool) => {
    ...
    toolDiv.innerHTML = `
            <div class="browse-item-info">
                <h4>${
                  tool.toolkit ? `Toolkit ${tool.toolkit}` : "No Toolkit"
                }</p>
                <h3>${tool.name}</h3>
                <p>${tool.desc}</p>
            </div>
...
// Render search results based on the search query
const renderSearchResults = (query) => {
  ...
  const filteredTools = browseTools.filter(
    (tool) =>
      tool.name.toLowerCase().includes(query.toLowerCase()) || // Check if tool name matches the query
      (tool.toolkit &&
        `Toolkit ${tool.toolkit}`.toLowerCase().includes(query.toLowerCase())) // Check if toolkit name matches the query
  );

  // Render search results
  filteredTools.forEach((tool) => {
    const toolDiv = document.createElement("div");
    toolDiv.className = "search-item";
    toolDiv.innerHTML = `
            <h3>${tool.name}</h3>
            <p>${tool.desc}</p>
        `;
    searchResults.appendChild(toolDiv);
  });

  // Render "No results found" message if no tools match the query
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Toggle the search tab on search icon click
